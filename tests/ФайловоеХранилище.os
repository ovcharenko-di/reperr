...
// BSLLS:UnusedParameters-on
	...
	ВсеТесты.Добавить("ТестДолжен_ОбработатьПовторноеПолучениеОтчетаПоСуществующейОшибке");
...
Процедура ТестДолжен_УстановитьИдЗадачиВТрекере() Экспорт
	...
	Ожидаем.Что(Ошибки.Количество()).Равно(3);
...
Процедура ТестДолжен_ОбработатьПовторноеПолучениеОтчетаПоСуществующейОшибке () Экспорт

	Каталог = "data";
	ХранилищеДанных = Новый ФайловоеХранилище(Каталог);

	ЧтениеДанных = Новый ЧтениеДанных("Ошибка_20220220230255.zip", "utf-8");
	РезультатЧтения = ЧтениеДанных.Прочитать();
	ЧтениеДанных.Закрыть();

	МенеджерХранилищаОшибок.Инициализировать (ХранилищеДанных, Каталог).ОбработатьОтчетОбОшибке (РезультатЧтения);
	...
	Ошибка = Ошибки[2];
	Ожидаем.Что(Ошибка.datetime).Равно(Дата ("20220220230255"));
