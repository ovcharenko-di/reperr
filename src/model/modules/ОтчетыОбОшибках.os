...
Функция СформироватьТелоОписанияОшибкиJSON(ДанныеОтчетаОбОшибке) Экспорт
	...
	ПримечаниеПользователя = ИнфоОбОшибке["userDescription"];
	Если ПримечаниеПользователя <> Неопределено Тогда
		ПоляОписанияОшибки.Вставить("ПримечаниеПользователя", ПримечаниеПользователя);
	КонецЕсли;
